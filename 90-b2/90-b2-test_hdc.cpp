...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_LINE] == '1') {
		...
		for (i = 0; i < 20; i++) {
			...
			y2 = rand() % win_high;
			/* 线的粗细 1~5，颜色随机 */
			hdc_line(x1, y1, x2, y2, rand()%5+1, RGB(rand()%256, rand()%256, rand()%256));
			Sleep(1);
			}
...
//				hdc_sector(350, 350, 200 - round * 20 + i * 10, t[i].angle_begin, t[i].angle_end, round, 2+rand()%5, RGB(rand() % 256, rand() % 256, rand() % 256)); //随机色
	...
	if (test_choise[T_MATHCURVE] == '1') {
		/* 画一条数学曲线，具体内容可自选（例：阿基米德螺线）
		   必做项 */
		hdc_cls();
		void hdc_draw_Mathematical_curve(int point_x, int point_y, int radius_a,int rotation_angles, const bool filled, const int thickness, const int RGB_value = INVALID_RGB);

		/* 先测试椭圆的旋转，目的是为了验证旋转的的正确性 */
		for (round = 0; round <= 0; round++)
			for (i = 0; i < 360; i += 15) {
				sprintf(tmp, "下面要显示%s心形，旋转 %d 度", fill_str[round % 2], i); //可以直接fill_str[round]，加round%2是为了防止溢出
				to_be_continued(tmp, 0, 0);
				hdc_draw_Mathematical_curve(350, 350,  200 - round * 20, i, round, 2 + rand() % 5, RGB(255, 155, 192));	//预置三色轮换
			}

		to_be_continued("继续测试随机画心形", STL_X, STL_Y);
		hdc_cls();
		...
		for (i = 0; i < 20; i++) {
			...
			b = win_high / 40 + rand() % (win_high / 4);
			angle = rand() % 360;
			hdc_draw_Mathematical_curve(x, y, b, angle, i % 2, 2 + rand() % 5);	//预置三色轮换
			Sleep(100);
		}
		to_be_continued("画心形线完成", STL_X, STL_Y);
	}
